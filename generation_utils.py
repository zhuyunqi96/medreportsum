...
class GenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        sampleprompt = None
        if "sampleprompt" in encoder_kwargs:
            sampleprompt = encoder_kwargs.get("sampleprompt")
            encoder_kwargs.pop("sampleprompt")

        model_kwargs["encoder_outputs"]: ModelOutput = encoder(**encoder_kwargs)

        # custom for prompt_signals ===========
        prompt_kwards = dict(encoder_kwargs)

        if sampleprompt is not None:
            _input_ids = sampleprompt
            _attn_mask = torch.where(sampleprompt != pad_token_id, 1, 0)
            _attn_mask = _attn_mask.type(torch.long)
        
            prompt_kwards["input_ids"] = _input_ids
            prompt_kwards["attention_mask"] = _attn_mask
            model_kwargs["prompt_signals"] = encoder(**prompt_kwards).last_hidden_state
            model_kwargs["prompt_attn_mask"] = _attn_mask
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        if is_encoder_decoder:
            ...
            if model_kwargs.get('prompt_signals') is not None:
                model_kwargs["prompt_signals"] = model_kwargs['prompt_signals'].index_select(0, expanded_return_idx)
                model_kwargs["prompt_attn_mask"] = model_kwargs['prompt_attn_mask'].index_select(0, expanded_return_idx)
        ...
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(
                inputs_tensor, model_kwargs, model_input_name, max_length, pad_token_id
            )
